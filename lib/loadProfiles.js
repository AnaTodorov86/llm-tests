 ...
 */
export function generateThresholds(profile, providerName = 'groq') {
    const networkBuffer = providerName === 'render-mock' ? 4000 : 0;
    const effectiveTTFT = profile.expectedTTFT + networkBuffer;

    return {
        'time_to_first_token': [
            `p(95)<${effectiveTTFT}`,
            `p(99)<${effectiveTTFT * 1.5}`,
        ],
        ...
        'llm_latency': [
            `p(95)<${effectiveTTFT + (profile.maxTokens * 20)}`,
            `p(99)<${effectiveTTFT * 1.5 + (profile.maxTokens * 30)}`,
        ],
        
        'stream_errors': ['rate<0.01'],         
        'llm_success': ['rate>0.95'],           
        'incomplete_responses': ['rate<0.05'],
    };
}
